...
/* sw.js — safe SPA worker for Vite + local media cache
   ------------------------------------------------------------
   - Bypasses ALL interception on localhost (preview/dev)
   - Never intercepts /assets/*.js|.css|.map (lets Vite serve bundles)
   - Network-first for navigations (index.html), cache fallback only
   - Keeps /local/* media cache with Range support for <video>
*/

const SW_VERSION = "2025-11-02.6";              // bump to force update
const PRECACHE_NAME = "kabuto-precache-v3";     // bump if URLS changes
const LOCAL_CACHE_NAME = "kabuto-local-v3";     // media cache
const DEV_BYPASS = ["localhost", "127.0.0.1"].includes(self.location.hostname);

// Keep your static assets here (NO index.html here)
const URLS = [
  // SFX
  "/sfx/click.mp3",
  "/sfx/transition.mp3",
...
/* ---------- install/activate ---------- */
self.addEventListener("install", (event) => {
  // Don’t precache on localhost to avoid preview issues
  if (DEV_BYPASS) { self.skipWaiting(); return; }
  event.waitUntil(
    caches.open(PRECACHE_NAME)
      .then((cache) => cache.addAll(URLS))
      .then(() => self.skipWaiting())
...
self.addEventListener("activate", (event) => {
  event.waitUntil((async () => {
    const keep = new Set([PRECACHE_NAME, LOCAL_CACHE_NAME]);
    const keys = await caches.keys();
    await Promise.all(keys.map((k) => (keep.has(k) ? undefined : caches.delete(k))));
    await self.clients.claim();
  })());
...
self.addEventListener("fetch", (event) => {
  ...
  // 0) Never intercept anything on localhost (vite preview/dev)
  if (DEV_BYPASS) return;

  // 1) Never intercept Vite hashed assets or source maps
  if (
    ...
    return; // let the network/Vite serve them
  }

  // 2) Local media cache: /local/* with Range support
  if (url.origin === location.origin && url.pathname.startsWith("/local/")) {
    event.respondWith((async () => {
      const cache = await caches.open(LOCAL_CACHE_NAME);
      const hit = await cache.match(url.pathname);
  ...
  // 3) Navigations (index.html): network-first (avoid stale HTML -> stale asset URLs)
  if (req.mode === "navigate") {
    event.respondWith((async () => {
      try {
        return await fetch(req);
      } catch {
        // fallback to cached shell if you precached one (we didn’t cache /index.html here)
        const cache = await caches.open(PRECACHE_NAME);
  ...
  // 4) Static files we chose to precache (images, sfx, etc.): cache-first
  if (req.method === "GET" && url.origin === location.origin) {
    event.respondWith((async () => {
      const cache = await caches.open(PRECACHE_NAME);
      const hit = await cache.match(url.pathname);
