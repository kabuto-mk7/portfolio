...
import { listPostsPublic } from "@/lib/storage";
...
export default function BlogIndexPage() {
  ...
  const refresh = React.useCallback(async () => {
    try {
      setLoading(true);
      const rows = await listPostsPublic();
      // newest first by date, then updatedAt
      rows.sort((a, b) =>
  ...
  React.useEffect(() => {
    void refresh();
    // refresh when Admin saves something that affects /blog
    const onData = (e: any) => {
      if (e?.detail?.type === "posts") void refresh();
    };
    window.addEventListener("kabuto:data", onData as any);
    return () => window.removeEventListener("kabuto:data", onData as any);
  }, [refresh]);
