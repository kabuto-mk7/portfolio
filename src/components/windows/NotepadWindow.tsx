...
export function NotepadWindow({ onClose }: { onClose: () => void }) {
  const bevelUp = { borderTop:"1px solid #fff", borderLeft:"1px solid #fff", borderRight:"1px solid #404040", borderBottom:"1px solid #404040" } as React.CSSProperties;
  const bevelDown = { borderTop:"1px solid #808080", borderLeft:"1px solid #808080", borderRight:"1px solid #fff", borderBottom:"1px solid #fff" } as React.CSSProperties;
  const [pos, setPos] = React.useState<{ x: number; y: number }>(() => {
    const w = typeof window !== "undefined" ? window.innerWidth : 1200; return { x: Math.max(8, (w - 520) / 2), y: 80 };
  });
  const dragging = React.useRef<{ ox: number; oy: number } | null>(null);
  React.useEffect(() => {
    const onMove = (e: MouseEvent) => { if (!dragging.current) return; setPos({ x: Math.max(0, e.clientX - dragging.current!.ox), y: Math.max(0, e.clientY - dragging.current!.oy) }); };
  ...
  return (
    <div className="fixed inset-0 z-[200] bg-black/30">
      <div className="bg-[#c0c0c0] fixed" style={{ ...bevelDown, width: 520, left: pos.x, top: pos.y }} onMouseDown={(e) => e.stopPropagation()}>
        <div onMouseDown={startDrag} className="h-7 flex items-center justify-between px-2 select-none" style={{ background:"#000080", borderBottom:"1px solid #000040", color:"#fff" }}>
